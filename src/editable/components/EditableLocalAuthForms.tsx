...
const inputClass =
  'h-12 rounded-full border border-[var(--fu-line)] bg-[var(--fu-soft)] px-5 text-sm font-medium text-[var(--fu-text)] outline-none transition placeholder:text-black/35 focus:border-[var(--fu-accent-ring)] focus:bg-white'
const buttonClass =
  'inline-flex h-12 items-center justify-center rounded-full bg-[var(--fu-accent)] px-6 text-sm font-semibold text-white transition duration-200 hover:bg-[var(--fu-accent-strong)] hover:-translate-y-0.5 disabled:opacity-60'
const messageClass = 'rounded-[var(--fu-radius-sm)] px-4 py-3 text-sm font-medium'
...
export function EditableLocalLoginForm() {
  ...
  return (
    <form className="mt-6 grid gap-4" onSubmit={submit}>
      ...
      {message ? <p className={`${messageClass} ${status === 'success' ? 'bg-emerald-50 text-emerald-800' : 'bg-red-50 text-red-700'}`}>{message}</p> : null}
